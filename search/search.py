...
# flag = 0
# def rundfs(state,path,problem,flag):
#     if(flag): return
...
#     for s in states:
#         path.append(s[1])
#         return rundfs(s[0],path+,problem,flag)
...
def uniformCostSearch(problem):
    ...
    while True:
        ...
        state , path , path_cost = pr.pop()
        if state in visited: continue
        visited.add(state)
        if(problem.isGoalState(state)): return path
        for child,action,cost in problem.getSuccessors(state):
            pr.push((child,path+[action],cost+path_cost),cost + path_cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while True:
        ...
        for child,action,cost in problem.getSuccessors(state):
            ...
            pr.push((child,path+[action],path_cost+h),path_cost+h)
